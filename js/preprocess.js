...
SHXParser.load = function(url, callback, returnData) {
    var xhr = new XMLHttpRequest();
    xhr.open('GET', url);
    xhr.responseType = 'arraybuffer';
    xhr.onload = function() {
        var shxData = new SHXParser().parse(xhr.response,url);
        compareData.push(shxData.records);
        callback(compareData, returnData);
        URL.revokeObjectURL(url);
    };
    xhr.onerror = onerror;
    xhr.send(null);
...
SHXParser.prototype.parse = function(arrayBuffer, url) {
    var o = {};
    var dv = new DataView(arrayBuffer);
    var idx = 0;
    o.fileName = url;
    ...
    while (idx < o.byteLength) {
        var record = {};
        record.offset = dv.getInt32(idx, false);
        idx += 4;
        record.length = dv.getInt32(idx, false);
        idx += 4;

        o.records.push(record.length);
    }
    return o;
...
Array.prototype.diff = function(a) {
    var $this = this;
    var list = this.filter(function(i) {return a.indexOf(i) < 0;});
    list.forEach(function(part, index) {
        list[index] = $this.indexOf(part);
    });
    return list;
};