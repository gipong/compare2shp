        ...
        if(typeof url != 'string') {
            ...
            reader.onload = function(e) {
                var URL = window.URL || window.webkitURL || window.mozURL || window.msURL,
                    ...
                    shxString = zip.file(/.shx$/i)[0].name;

                if(prjString) {
                    proj4.defs('EPSGUSER', zip.file(prjString.name).asText());
                    EPSGUser = proj4('EPSGUSER');
                }
                SHXParser.load(URL.createObjectURL(new Blob([zip.file(shxString).asArrayBuffer()])), shxLoader, returnData);
...
function shpLoader(data, returnData) {
    inputData['shp'] = data;
    if(inputData['shp'] && inputData['dbf'] && inputData['shx']) {
        if(returnData) returnData(  toGeojson(inputData), inputData['shx']  );
    }
}
...
function shxLoader(data, returnData) {
    inputData['shx'] = data;
    if(inputData['shp'] && inputData['dbf'] && inputData['shx']) {
        if(returnData) returnData(  toGeojson(inputData), inputData['shx']  );
    }
